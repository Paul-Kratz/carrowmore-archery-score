...
export class ShootParticipant {
  ...
  constructor(
    participantData: IDenormalizedParticipant,
...
export class Shoot {
  ...
  get guestCount() {
    return this.participants.filter((participant) => participant.isGuest)
      .length;
  }
  ...
  get completedStationCount() {
    ...
    return Array.from({ length: this.totalStations }, (_, stationIndex) =>
      this.participants.every((participant) =>
        participant.hasScoreForStation(stationIndex + 1),
      ),
  ...
  getStationCompletionCount(station: number) {
    return this.participants.filter((participant) =>
      participant.hasScoreForStation(station),
    ).length;
  }
  ...
  getNextUnscoredParticipant(participantId: string, station: number) {
    ...
    return (
      Array.from({ length: this.participants.length - 1 }, (_, offset) => {
        const nextIndex =
          (participantIndex + offset + 1) % this.participants.length;
